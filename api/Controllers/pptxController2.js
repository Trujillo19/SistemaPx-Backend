...
exports.getPptx = async (req, res, next) => {
    ...
    try {
        ...
        // Busca en la base de datos los datos de la gráfica ejercicio.
        var exerciseChartRes = await exerciseChart.find({exerciseDate: {$gte: startDate, $lte:endDate}});
        // Verificación de información en BD
        ...
        // Variables globales
        //
        // Create a presentation and the layout.
        let pres = new pptxgen();
        pres.layout = 'LAYOUT_4x3';
        // Array to convert date from 0 to 11 to a month.
        const MESES = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
        // Recuperar la fecha del último ejercicio y dividirla en día, mes y año.
        var fechaActual = exercise[exercise.length-1].createdAt;
        ...
        // El mes inicial y mes final de la presentación
        var mesInicial = MESES[moment(startDate).month()];
        var mesFinal = MESES[moment(endDate).month()];
        // Adecuado y Ejercicio
        var e_AA = 0;
        var e_CGDUOS = 0;
        var e_GMDE = 0;
        var e_GMGE = 0;
        ...
        var avance = numeral(realTableSuma / adecSumaAvance).format('0%');
        ...
        for (let i = 0; i < exercise.length; i++) {
            realChart[i] = exercise[i].AA + exercise[i].CGDUOS + exercise[i].GMDE + exercise[i].GMGE + exercise[i].GMM
            + exercise[i].GMOPI + exercise[i].CSTPIP + exercise[i].GSMCCIT + exercise[i].GSSLT + exercise[i].GMSSTPA;
        ...
        var graficaRealvsAdecuado = [];
        var ejeReal = { name:'REAL', labels:Meses, values:realChart};
        var ejeAdecuado = { name:'ADEC III', labels:Meses, values:adecChart};
        graficaRealvsAdecuado.push( ejeReal );
        graficaRealvsAdecuado.push( ejeAdecuado );
        var optsChartLine1 = {
            x:0.5,
            y:1.6,
            w:9.0,
        ...
        slide3.addChart( pres.charts.LINE, graficaRealvsAdecuado, optsChartLine1);
        ...
        slide3.addText(avance, { shape:pres.shapes.OVAL, x:2, y:5.6, w:1.4, h:0.73, fill:'FF0000', color:'FFFFFF', align:'center', fontSize:21,bold:true });
